...
def fuc_name(func, *args):
    readeble_name = f'{func.__name__.replace("_", " ").title()} [{", ".join(args)}]'
    print(readeble_name)
    return readeble_name


def open_browser(browser_name):
    actual_result = fuc_name(open_browser, browser_name)
    assert actual_result == "Open Browser [Chrome]"


def go_to_companyname_homepage(page_url):
    actual_result = fuc_name(go_to_companyname_homepage, page_url)
    assert actual_result == "Go To Companyname Homepage [https://companyname.com]"


def find_registration_button_on_login_page(page_url, button_text):
    actual_result = fuc_name(
